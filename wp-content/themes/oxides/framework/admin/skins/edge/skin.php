...
class EdgeOxidesSkin extends EdgeOxidesSkinAbstract {
    /**
     * Skin constructor. Hooks to oxides_edge_admin_scripts_init and oxides_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('oxides_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('oxides_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('oxides_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('oxides_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('oxides_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('oxides_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            oxides_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            oxides_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = oxides_edge_get_admin_tab();
        $active_page = $oxides_edgeFramework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     * @param EdgeOxidesAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
