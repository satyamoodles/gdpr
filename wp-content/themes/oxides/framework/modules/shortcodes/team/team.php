<?php
namespace EdgeOxidesfModules\Team;
...
class Team implements ShortcodeInterface
{
	...
	public function vcMap()	{
		...
		for ($x = 1; $x<7; $x++) {
			$teamIconCollections = oxides_edge_icon_collections()->getCollectionsWithSocialIcons();
			...
			$team_social_icons_array[] = array(
				...
				'dependency' => array('element' => 'team_social_icon_pack', 'value' => oxides_edge_icon_collections()->getIconCollectionsKeys())
			);
		...
		vc_map( array(
			'name' => esc_html__('Edge Team', 'oxides'),
			'base' => $this->base,
			'category' => 'by EDGE',
			...
			'params' => array_merge(
				array(
					...
					array(
						...
						'admin_label' => true,
						'value' => array_merge(array('' => ''),oxides_edge_icon_collections()->getIconCollectionsVCExclude(array('linea_icons', 'dripicons'))),
						'save_always' => true
					),
					array(
						'type' => 'dropdown',
	...
	public function render($atts, $content = null)
	{
		...
		$params['number_of_social_icons'] = 6;
        ...
        $params['team_link']   = !empty($params['team_link']) ? $params['team_link'] : '';
        $params['team_target'] = !empty($params['team_target']) ? $params['team_target'] : '_self';

		//Get HTML from template based on type of team
		$html = oxides_edge_get_shortcode_module_template_part('templates/' . $params['team_type'], 'team', '', $params);
	...
	private function getTeamSocialIcons($params) {
		...
		if ($team_social_icon_pack !== '') {
			...
			$icon_pack = oxides_edge_icon_collections()->getIconCollection($team_social_icon_pack);
			...
			for ( $i = 1; $i <= $number_of_social_icons; $i++ ) {
				...
				if ($team_social_icon !== '') {
					...
					$social_icons[] = oxides_edge_execute_shortcode('edgtf_icon', $team_icon_params);
				}
