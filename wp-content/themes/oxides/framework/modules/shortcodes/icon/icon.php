<?php
namespace EdgeOxidesfModules\Shortcodes\Icon;

use EdgeOxidesfModules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package EdgeOxidesfModules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Edge Icon', 'oxides'),
            'base'                      => $this->base,
            'category'                  => 'by EDGE',
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        ...
                        'description' => 'Please insert border radius(Rounded corners) in px. For example: 4.',
    ...
    public function render($atts, $content = null) {
        ...
        $html = oxides_edge_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.oxides_edge_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.oxides_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.oxides_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.oxides_edge_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                ...
                $iconHolderStyles[] = 'border-width: '.oxides_edge_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.oxides_edge_filter_px($params['border_radius']).'px';
                }
            }
        }
