...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(oxides_edge_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	$inner_start .= '<div class="edgtf-vc-row-animation-holder '. $css_animation .'">';
	if($transition_delay !== ''){
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . oxides_edge_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	
	$video_output .= '<div class=' . oxides_edge_get_class_attribute($video_overlay_class) . '"' . oxides_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';	

	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. oxides_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . oxides_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="Video thumb" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
