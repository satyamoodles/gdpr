...
if(!function_exists('edgtf_core_portfolio_ajax_load_more')){
	...
	if(!empty($_POST['display_title'])){
        $shortcode_params['display_title'] = $_POST['display_title'];
    }
    if(!empty($_POST['display_excerpt'])){
        $shortcode_params['display_excerpt'] = $_POST['display_excerpt'];
    }
    if(!empty($_POST['excerpt_length'])){
        $shortcode_params['excerpt_length'] = $_POST['excerpt_length'];
    }
    if(!empty($_POST['display_separator'])){
        $shortcode_params['display_separator'] = $_POST['display_separator'];
    }
    if(!empty($_POST['display_category'])){
        $shortcode_params['display_category'] = $_POST['display_category'];
    }
    if(!empty($_POST['display_link'])){
        $shortcode_params['display_link'] = $_POST['display_link'];
    }
    if(!empty($_POST['display_lightbox'])){
        $shortcode_params['display_lightbox'] = $_POST['display_lightbox'];
    }
    if(!empty($_POST['space_between_items'])){
        $shortcode_params['space_between_items'] = $_POST['space_between_items'];
    }
    if(!empty($_POST['image_size'])){
        $shortcode_params['image_size'] = $_POST['image_size'];
    }
	...
	if($query_results->have_posts()):			
		while ( $query_results->have_posts() ) : $query_results->the_post(); 
            ...
            $shortcode_params['separator_html'] = $port_list->getItemSeparatorHtml($shortcode_params);
            $shortcode_params['excerpt_html'] = $port_list->getItemExcerptHtml($shortcode_params);
